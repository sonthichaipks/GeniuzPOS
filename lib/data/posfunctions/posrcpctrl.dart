...
class PosRCPCtrl {
  ...
  Future<double> addSalesItem(BuildContext context, SalesItems sales) async {
    var inventoryDb = Provider.of<ReceiptItemHiveModel>(context, listen: false);
  ...
  Future<double> voidARCPItem(BuildContext context, int index) async {
    var inventoryDb = Provider.of<ReceiptItemHiveModel>(context, listen: false);
    double amount;
    if (index == -1) {
      amount = 0;
      ...
      amount = await inventoryDb.getLastRCPamount();
