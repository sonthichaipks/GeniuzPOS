...
class ReceiptsItem extends StatefulWidget {
  const ReceiptsItem({
    Key key,
  }) : super(key: key);
  @override
  _ReceiptsItem createState() => _ReceiptsItem();
}

class _ReceiptsItem extends State<ReceiptsItem> {
  @override
  void dispose() {
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<ReceiptItemHiveModel>(builder: (context, model, child) {
      return Row(
        children: [
          GestureDetector(
            // onPanUpdate: (details) {
  ...
  List<DataRow> saleslistFB(ReceiptItemHiveModel model) {
    var sallist = model.inventoryList
        .where((element) => PosRCPCtrl().checkRCP(element))
        .toList();
    int slcnt;
